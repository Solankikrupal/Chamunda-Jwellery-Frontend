...
import { AxiosPost } from "../configs/common/axiosService";
import { doLogin, loginFilepath } from "../Auth";
...
const LoginPage = (props: Props) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response && typeof response !== 'string' && response.statusCode === 200 && response?.data[0]?.role?.roleName === 'Employee') {
        console.log('response====>', response)
        await loginFilepath(response?.filePath);
        await doLogin(response.data[0],'userSession'); // Perform login
